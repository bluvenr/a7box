...
| 为了 10 个不同任务安装 10 个独立应用 | 22 款工具，一个 ~10MB 的应用 |
...
- **剪贴板管理** —— 可搜索、可置顶的剪贴板历史（本地加密），快捷粘贴弹窗（`Alt+V`）、批量粘贴、文本片段与自动化规则
...
内置 22 款工具，支持侧边栏、命令面板（`Ctrl+K`）和自定义全局快捷键三种方式调用。
...
| **剪贴板管理** | 加密剪贴板历史，支持搜索/置顶、快捷粘贴弹窗、批量粘贴、文本片段与自动化规则 |
