/**
 * Code Minify History Panel
 */

import { useState } from 'react'
import { useTranslation } from 'react-i18next'
import { Clock, Trash2, X, RotateCcw } from 'lucide-react'
import type { Language } from '../utils/minifier'

export interface HistoryItem {
  id: string
  input: string
  output: string
  action: 'compress' | 'beautify'
  language: Language
  timestamp: number
}
...
export function HistoryPanel({ items, onRestore, onClear, onClose }: HistoryPanelProps) {
  ...
  function formatTime(timestamp: number): string {
    ...
    if (minutes < 1) return t('modules.codeMinify.ui.timeJustNow', { defaultValue: 'Just now' })
    if (minutes < 60) return t('modules.codeMinify.ui.timeMinutesAgo', { m: minutes, defaultValue: `${minutes}m ago` })
    const hours = Math.floor(minutes / 60)
    if (hours < 24) return t('modules.codeMinify.ui.timeHoursAgo', { h: hours, defaultValue: `${hours}h ago` })
      ...
      <div className="flex items-center justify-between border-b border-border-subtle px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-text-primary">
            {t('modules.codeMinify.ui.historyTitle', { defaultValue: 'History' })}
          </span>
        ...
        <div className="flex items-center gap-2">
          {items.length > 0 && (
            ...
            >
              ...
              {t('modules.codeMinify.ui.historyClear', { defaultValue: 'Clear' })}
            </button>
          )}
          <button
            onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-text-muted">
            ...
            <p className="text-sm">
              {t('modules.codeMinify.ui.historyEmpty', { defaultValue: 'No history yet' })}
            </p>
          </div>
        ) : (
          items.map((item) => (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {item.action === 'beautify'
                        ? t('modules.codeMinify.ui.historyActionBeautify', { defaultValue: 'Beautify' })
                        : t('modules.codeMinify.ui.historyActionCompress', { defaultValue: 'Compress' })}
                    </span>
                    <span className="rounded bg-bg-hover px-1 py-0.5 text-[10px] text-text-muted">
...
const HISTORY_KEY = 'a7box-code-minify-history'
