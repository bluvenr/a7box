...
import {
  type ViewMode,
  MAX_DIFF_LINES,
  wordDiff, computeDiff, computeModifiedPairs, computeStats,
  formatUnified, formatSplit,
} from './utils'
...
export default function TextDiff() {
  ...
  const handleDragLeave = () => { setDragSide(null) }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden p-6">
      ...
      <div className="mb-5 flex items-center justify-between">
        ...
        {(left || right) && (
          <button onClick={clearAll} className="flex items-center gap-1.5 rounded-lg bg-bg-hover px-3 py-2 text-sm text-text-secondary transition hover:bg-bg-elevated hover:text-text-primary cursor-pointer">
            <X size={14} />
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <div className="flex items-center self-center">
          <button onClick={swapTexts} className="flex h-8 w-8 items-center justify-center rounded-full border border-border-subtle text-text-muted transition hover:border-primary hover:text-primary cursor-pointer" title={t('modules.textDiff.swap')}>
      ...
      {diffs.length > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-3 text-sm">
          ...
          {stats.added > 0 && (
            <span className="rounded bg-green-500/10 px-2 py-0.5 text-green-400">+{stats.added} {t('modules.textDiff.linesAdded')}</span>
          )}
          {stats.removed > 0 && (
            <span className="rounded bg-red-500/10 px-2 py-0.5 text-red-400">-{stats.removed} {t('modules.textDiff.linesRemoved')}</span>
          )}
        ...
        <div className="flex flex-1 flex-col overflow-hidden rounded-xl border border-border-subtle bg-bg-elevated">
          <div className="flex items-center justify-between border-b border-border-subtle px-4 py-2">
            <div className="flex overflow-hidden rounded-md border border-border-subtle w-fit">
              <button onClick={() => setViewMode('unified')} className={`px-3 py-1 text-xs font-medium transition-colors ${viewMode === 'unified' ? 'bg-primary/10 text-primary' : 'text-text-muted hover:bg-bg-hover'}`}>
                {t('modules.textDiff.viewUnified', { defaultValue: 'Unified' })}
              </button>
              <button onClick={() => setViewMode('split')} className={`px-3 py-1 text-xs font-medium transition-colors ${viewMode === 'split' ? 'bg-primary/10 text-primary' : 'text-text-muted hover:bg-bg-hover'}`}>
                {t('modules.textDiff.viewSplit', { defaultValue: 'Split' })}
              </button>
            </div>
            <button onClick={copyResult} className="flex items-center gap-1 rounded px-2 py-1 text-xs text-text-muted transition hover:text-primary cursor-pointer">
              ...
              <table className="w-full">
                <tbody>
                  {diffs.map((d, i) => {
                    ...
                    return (
                      <tr key={i} className={`border-b border-border-subtle/30 ${d.type === 'added' ? 'bg-green-500/8' : d.type === 'removed' ? 'bg-red-500/8' : ''}`}>
                        ...
                        <td className="px-4 py-1 font-mono text-sm whitespace-pre-wrap">
                          {isModified ? (
                            ...
                            <span className={d.type === 'added' ? 'text-green-300' : d.type === 'removed' ? 'text-red-300' : 'text-text-secondary'}>{d.text || ' '}</span>
                          )}
                        </td>
                      </tr>
                    )
                      ...
                      return (
                        <tr key={i} className="border-b border-border-subtle/30">
                          ...
                          <td className="w-1/2 bg-red-500/8 px-4 py-1 font-mono text-sm whitespace-pre-wrap overflow-hidden">
                            <span className="text-red-300">{d.text.split(/(\s+)/).map((p, pi) => <span key={pi} className={oldChanged.has(p) ? 'bg-red-500/20 rounded-sm' : ''}>{p}</span>)}</span>
                          </td>
                          ...
                          <td className="w-1/2 bg-green-500/8 px-4 py-1 font-mono text-sm whitespace-pre-wrap overflow-hidden">
                            <span className="text-green-300">{next.text.split(/(\s+)/).map((p, pi) => <span key={pi} className={newChanged.has(p) ? 'bg-green-500/20 rounded-sm' : ''}>{p}</span>)}</span>
                          </td>
                        </tr>
                      )
                    }
